import type { NextPage } from "next";

const Footer: NextPage = () => {
  return (
    <div className="w-full bg-text-colors-dark-gray max-w-full flex flex-col items-center justify-start p-2.5 box-border text-center text-17xl text-text-colors-amber font-headings-headling-2">
      <div className="w-full flex flex-row flex-wrap items-start justify-start gap-[10px] max-w-[1300px]">
        <b className="relative leading-[40px]">TODO: FOOTER</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
        <b className="relative leading-[40px]">Footer Item1</b>
      </div>
    </div>
  );
};

export default Footer;
